function [n_m, tau_m, T_m] = two_point_Strejc(t, h, stop_time)
    ...
    T_90 = t(indeks); % znajdowanie chwili czasowej T90
    
    for iter = 1:6
    
        diff = abs(h - h_n(iter));
        [min_diff, indeks] = min(diff);
        t_vec(iter) = t(indeks); % znajdowanie chwil czasowych t02, t03 ... t06
        T_vec(iter) = T_vec_coef(iter) * ((T_90) - t_vec(iter)); % obliczanie stałych czasowych
        tau_vec(iter) = t_vec(iter) - iter*T_vec(iter); % obliczanie opóźnień obiektów
    end 
    ...
    "Znalezione najlepsze parametry obiektu"
    n_m = min_index + 1
